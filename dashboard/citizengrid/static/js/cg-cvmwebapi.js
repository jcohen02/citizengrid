...
function initCVMWebAPISession(app_id) {
		window.plugin.requestSession('http://test.local:8000/vmcp?appid=' + app_id, function(session) {
			...
			session.addEventListener('stateChanged', function(code) {
				...
				if(code == 5) {
					$('#launch-app-client-btn').html('Terminate Local Virtual Machine');
				}
				else if(code < 5) {
					$('#launch-app-client-btn').html('Launch Application Client Locally');
				}
			});
